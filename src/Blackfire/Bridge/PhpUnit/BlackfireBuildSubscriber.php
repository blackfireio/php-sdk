...
final class BlackfireBuildSubscriber
{
    ...
    public function executeAfterLastTest(): void
    {
        ...
        if (0 === $build->getScenarioCount()) {
            echo "\n";
            echo 'Blackfire: No scenario was created for the current build.';
            echo "\n";
        ...
        if ($this->buildHelper->hasAnyScenario()) {
            echo "\n";
            echo 'Blackfire: The last scenario was not ended.';
            echo "\n";
        ...
        try {
            if ($report->isErrored()) {
                echo "\n";
                echo 'Blackfire: The build has errored';
                echo "\n";
            } else {
                if ($report->isSuccessful()) {
                    echo "\n";
                    echo 'Blackfire: The build was successful';
                    echo "\n";
                } else {
                    echo "\n";
                    echo 'Blackfire: The build has failed';
                    echo "\n";
                }
            }
    ...
    public function executeAfterTest(string $className): void
    {
        if (!method_exists($className, 'isBlackfireScenarioAutoStart')) {
            return;
        }

        if (true === call_user_func("$className::isBlackfireScenarioAutoStart")) {
            return;
        }
